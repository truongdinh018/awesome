...
> **Category:** MCP & AI Agents · Production engineering skills pack  
...
**Addy's Agent Skills** — 24 **production-grade** skills + 8 slash commands (`/spec` → `/ship`) cho AI coding agents (Claude Code, Cursor, Codex, Copilot, Gemini, OpenCode…). Workflow có gate: Define → Plan → Build → Verify → Review → Ship. Không phải runtime agent — là **SKILL.md pack** (spec/TDD/security/webperf…) cài vào agent bạn đã dùng.

Peer: [Ponytail](ponytail.md) (YAGNI skill mỏng), [CLI-Anything](cli-anything.md) / [HTML Anything](html-anything.md) (skill domain-specific), [Hermes](hermes-agent.md) (agent runtime + skills), [prompts.chat](prompts-chat.md) (prompt library).
